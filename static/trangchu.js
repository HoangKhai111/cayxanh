  ...
  fetch(getFeatureInfoUrl(e.latlng))
    ...
    .then(data => {
      if (data.features && data.features.length > 0) {
        ...
        const FID2 = props.FID2 || "Không rõ";
...
if (searchAct && searchInput) {
  searchAct.addEventListener('click', () => {
    ...
    fetch(`http://localhost:5500/api/cayxanh/timkiem_stt?FID=${encodeURIComponent(FID)}`)
      .then(res => {
...
map.on(L.Draw.Event.CREATED, function (e) {
  ...
  fetch('http://localhost:5500/api/cayxanh/trongvung', {
    method: 'POST',
    ...
    .then(data => {
      if (data.error) {
        ...
        console.log(data.trees); // hiện thị dữ liệu cây để in bảng CSV
        // Lưu dữ liệu vào biến để sử dụng trong hàm xuatCSV
